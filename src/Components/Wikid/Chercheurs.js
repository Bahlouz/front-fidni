...
// Function to check if the description contains the <chercheur> tag
const containsChercheurTag = (description) => {
    return description.some(paragraph => 
        paragraph.children.some(child => child.text === '<chercheur>')
    );
};
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter items based on subcategory 'WikiPhédia' and the presence of <chercheur> tag in description
                const filteredItems = data.data.filter(item => 
                    item.attributes?.subcategory?.data?.attributes?.name === 'WikiPhédia' &&
                    containsChercheurTag(item.attributes?.Description || [])
                );
